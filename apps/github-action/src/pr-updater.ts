...
function buildLazyprSection(options: LazyprSectionOptions): string {
  const { summary, riskLevel, impactScore, prTitle } = options;
  ...
  const section = `${LAZYPR_SECTION_START}\n## 🚀 PR Summary${titlePart}\n\n${summary}\n\n### 📊 Risk Assessment: ${riskEmoji} ${riskLevel} (Score: ${String(impactScore)}/100)\n\n---\n*Generated by lazypr at ${timestamp}*\n*View changes: ${prUrl}*${LAZYPR_SECTION_END}`;
...
export function formatGhostCommitsForMarkdown(
  ...
  const lines = ["### ⚠️ Potential Ghost Commits Detected"];
